...
p(warning). Requires Textpattern 4.6.0+ and "smd_crunchers":http://stefdawson.com/smd_crunchers plugin for import/export

Download the plugin from either "GitHub":https://github.com/Bloke/smd_admin_themes/, or the "software page":http://stefdawson.com/sw, paste the code into the Textpattern _Admin->Plugins_ pane, install and enable the plugin. A new tab labelled _Extensions->Admin themes_ is created. *Only publishers will see this tab*.

The plugin should automatically install the preferences for you. Click 'Preferences' from the _Extensions->Admin themes_ tab or _Options_ from the _Admin->Plugins_ tab to alter, delete or reinstall them.
...
On the _Extensions->Admin themes_ tab is a list or grid containing installed themes. Out of the box, there will be two available: _hive_ and _hiveneutral_. *It's advisable not to change these themes* so you can always go back to them if everything goes sideways. Plus it's a very useful starting point from which to "base":#clone a "new theme":#new.
...
Next is an upload area allowing you to upload new files and images to your theme. You can browse for as many files to upload as you wish -- subject to the maximum file size restriction as set in your _Advanced Preferences_ -- and (optionally) store them in the nominated folder. These can be any of the supported file types, including images.
